...
create_optionals = function(ens_select="")
{
  options_str <- ""
  if(ens_select != "")
  {
    options_str <- str_c(options_str, "&ens_select=", ens_select)
...
#Timeseries
timeseries <- function(startdate, enddate, interval, parameters, coordinate,
  plot = FALSE, model="mix", save_png = FALSE,
  target_directory = paste(getwd(),"timeseries.png",sep ="/"),
  plot_directory=getwd(), ens_select="")
{
  ...
  query = sprintf("https://%s:%s@api.meteomatics.com/%s--%s:%s/%s/%s/csv?model=%s%s",
                  username, password, startdate_query, enddate_query, interval,
                  parameters, coordinate, model, create_optionals(ens_select))
  resp1 <- GET(query)

  content <- content(resp1,"text")
  ...
  if (resp1$status_code != 200) {return(result$content)}  else {
    ...
    if (number_coordinates == 1) {
      ...
      colnames(df_timeseries) <- c(header[[1]][])
    }
